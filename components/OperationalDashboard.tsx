...
// Helper to get shared definitions (Shared across all accounts via localStorage)
...
const getPIDefinitions = (year: string, userId: string, role: UserRole) => {
  // Base hardcoded activities with stable IDs
  const baseDefinitions = [
    ...
    },
    {
      ...
      activities: [
        { id: "pi2_a1", name: "collaborative efforts with NGOs, CSOs, GAs and Non-GAs and other stakeholders activities", indicator: "No. of collaborative efforts with NGOs... activities conducted", defaults: [46, 43, 33, 33, 34, 35, 27, 26, 27, 27, 10, 25] }
      ]
    },
    {
      id: "PI3",
  ...
  // Extend base definitions with custom rows from localStorage
  return baseDefinitions.map(pi => {
    // Check if this PI has been deleted globally (optional future feature, but we stick to activity management)
  ...
  return definitions.map((def) => {
    ...
    return {
      ...
      activities: def.activities.map((act) => {
        ...
        return {
          id: act.id, // Ensure ID is passed for management
...
const OperationalDashboard: React.FC<OperationalDashboardProps> = ({ title = "OPERATIONAL DASHBOARD 2026", onBack, currentUser, subjectUser }) => {
  ...
  const isOperational2026 = is2026 && title.includes("OPERATIONAL DASHBOARD") && !title.includes("CHQ");
  ...
  const refreshData = () => {
    ...
    setDataMode(mode);
    setPiData(generateStructuredPIs(dashboardYear, subjectUser, mode));
  };
  ...
  const handleExportPPT = async () => {
    ...
    try {
      ...
      piData.forEach(pi => {
        ...
        // Define tableData with any[][] to avoid strict inference mismatch between header objects and data strings
        const tableData: any[][] = [[
