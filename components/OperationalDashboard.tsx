...
const getSharedPITitle = (prefix: string, year: string, userId: string, piId: string, defaultTitle: string): string => {
  const key = `${prefix}_pi_title_${year}_${userId}_piId}`;
...
const OperationalDashboard: React.FC<OperationalDashboardProps> = ({ title, onBack, currentUser, subjectUser, allUnits = [] }) => {
  ...
  const handleExportExcel = () => {
    ...
    currentPI.activities.forEach(act => {
      act.months.forEach((m, i) => { monthlyGrandTotals[i] += m.value; });
      overallGrandTotal += act.total;
    });
    const summaryRow: any = { 'Activity': 'GRAND TOTAL', 'Performance Indicator': 'Summary Total for current PI' };
    MONTHS.forEach((m, i) => { summaryRow[m] = monthlyGrandTotals[i]; });
    summaryRow['Total'] = overallGrandTotal;
    exportData.push(summaryRow);

    const ws = XLSX.utils.json_to_sheet(exportData);
    const wb = XLSX.utils.book_new();
    const sheetName = getSharedTabLabel(prefix, year, effectiveId, activeTab, activeTab);
  ...
  const handleExportAllTabs = () => {
    ...
    piData.forEach(pi => {
      ...
      // Calculate Summary Totals for each Tab/PI
      const monthlyGrandTotals = Array(12).fill(0);
      let overallGrandTotal = 0;
      ...
      // Append Summary Grand Total row to this sheet
      const grandTotalRow: any = { 'Activity': 'GRAND TOTAL', 'Performance Indicator': `Summary for ${pi.id}` };
      MONTHS.forEach((m, i) => { grandTotalRow[m] = monthlyGrandTotals[i]; });
      grandTotalRow['Total'] = overallGrandTotal;
      sheetData.push(grandTotalRow);

      const ws = XLSX.utils.json_to_sheet(sheetData);
      const tabName = getSharedTabLabel(prefix, year, effectiveId, pi.id, pi.id);
      XLSX.utils.book_append_sheet(wb, ws, tabName.substring(0, 31));
    });
    const cleanName = subjectUser.name.replace(/\s+/g, '_');
    XLSX.writeFile(wb, `${cleanName}_All_PI_Tabs_with_Totals_${year}.xlsx`);
  };
